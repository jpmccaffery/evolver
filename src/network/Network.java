...
private static List inputNeurons;
private static List outputNeurons;
...
private ArrayList<Connection> connsToAdd = new ArrayList<Connection>();
...
public void induceHeterosynapticLTD() {
	...
	while ( neuronIt.hasNext() ) {
		...
		Neuron n = (Neuron) neuronIt.next();
		ArrayList conns = n.getAfferentConnections();
		Iterator connIt = conns.iterator();		
		//Checks to see if at least one presynaptic connection
		//was increased via STDP
		while ( connIt.hasNext() ) {
			Connection c = (Connection) connIt.next();
		...
		Iterator connIt2 = conns.iterator();
		while ( connIt2.hasNext() ) {
			Connection c = (Connection) connIt2.next();
		...
		Iterator connIt3 = conns.iterator();		
		while ( connIt3.hasNext() ) {
			Connection c = (Connection) connIt3.next();
...
public void activate(double[] input) {
	...
	Iterator inputNeuronIt = inputNeurons.iterator();
	int counter = 0;
	while ( inputNeuronIt.hasNext() ) {
		Neuron n = (Neuron) inputNeuronIt.next();
		double totalInput = 0.0;
		totalInput += input[counter];
		ArrayList inboundConns = n.getAfferentConnections();
		Iterator connIterator = inboundConns.iterator();
		while ( connIterator.hasNext()) {
			Connection c = (Connection) connIterator.next();
			double preSynapticActivation = getNeuronActionPotentialByID( c.getPresynapticId() );
	...
	Iterator neuronIt2 = neurons.iterator();
	while ( neuronIt2.hasNext() ) {
		Neuron n = (Neuron) neuronIt2.next();
	...
	Iterator neuronIt4 = neurons.iterator();
	while ( neuronIt4.hasNext() ) {
		Neuron n = (Neuron) neuronIt4.next();
		if ( n.getType().toString().equals("delay")) {
			...
			if ( thisDelaySize < currentStep ) {
				...
				while ( neuronIt5.hasNext() ) {
					Neuron n2 = (Neuron) neuronIt5.next();
					if ( n2.getId() == thisParentId ) {
						double delayedMembranePotential = n2.getPreviousMembranePotential(thisDelayIdx - 1);
	...
	Iterator neuronIt3 = neurons.iterator();
	while ( neuronIt3.hasNext() ) {
            Neuron n = (Neuron) neuronIt3.next();
...
private void updateViaWinnerTakeAll() {	
	...
	while ( neuronCounter > 0 ) {
		...
		ArrayList neuronsInSameLayer = new ArrayList();
		Iterator neuronIt = neurons.iterator();
		while ( neuronIt.hasNext() ) {
			Neuron n = (Neuron) neuronIt.next();
			//if ( ( n.getLocation()[1] == currentY ) && ( n.getType().equals("output") || n.getType().equals("hidden") || n.getType().equals("input"))  ) {
			if ( ( n.getLocation()[1] == currentY ) && ( n.getType().equals("output") || n.getType().equals("hidden"))  ) {
				neuronsInSameLayer.add(n);
			}
		}
		ArrayList neuronThatJustFired = new ArrayList();		
        if (type.equals("leakyIntegrateAndFire")) {
            ...
            //to fire
            Iterator neuronIt2 = neuronsInSameLayer.iterator();
            while ( neuronIt2.hasNext() ) {
                    Neuron n = (Neuron) neuronIt2.next();
            ...
            if ( !neuronThatJustFired.isEmpty() ) {
                ...
                Iterator neuronIt3 = neuronsInSameLayer.iterator();
                while ( neuronIt3.hasNext() ) {
                        Neuron n = (Neuron) neuronIt3.next();
                        n.inhibitNeuron();
                        neuronCounter--;
        ...
        if (type.equals("leakyIntegrator")) {
            ...
            while ( neuronIt2.hasNext() ) {
                ...
                if (currentNeuronActivation >= 0) {
                	totalLayerActivation += (max * Math.pow(currentNeuronActivation, exp)) / ( Math.pow(midpointToMax, exp) + Math.pow(currentNeuronActivation, exp));                                            	
                }
            } 
            Iterator neuronIt3 = neuronsInSameLayer.iterator();          
            while ( neuronIt3.hasNext() ) {
                ...
                if (currentNeuronActivation >= 0) {
                	currentNeuronTransformedActivation = (max * Math.pow(currentNeuronActivation, exp)) / ( Math.pow(midpointToMax, exp) + Math.pow(currentNeuronActivation, exp));                 	
                }
